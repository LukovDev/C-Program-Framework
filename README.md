...
8. Проверка флагов в метаданных. Если флаги прошлой сборки отличаются от текущих - Сброс сборки (рекомпиляция всех файлов).
9. Обработка файлов:
...
- Удаляет те объектные файлы, исходник которых удалён. Также удаляет лишние объекты, которых не должно быть (очистка от мусора). Она отслеживает платформу, флаги сборки, файлы исходников и их время изменения. Делает рекомпиляцию (сброс сборки) в случае изменения во флагах сборки.
